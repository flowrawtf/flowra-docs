...
icon: law
order: 85
description: "Sub-slot mini-auctions, conflict-aware selection, atomic execution, and why open competition raises tips."
---

# Auction Mechanics

Flowra runs a continuous sequence of **sub-slot mini-auctions** aligned with Solana's continuous block production. This page explains the auction lifecycle, the selection rules, and why open competition raises validator revenue.
...
5. Each auction tick closes: the highest-tip, non-conflicting set wins and is forwarded to the leader, where it lands **first** in the block. Remaining block space fills under standard fee rules.

### Why ticks shorter than a slot?

Solana produces a block every ~400&nbsp;ms, continuously. Ticks much shorter than a slot fit multiple competitive cycles inside every block, so opportunities are contested close to the moment they appear rather than batched into one coarse round. Short windows also blunt pure latency racing: within a window, the *tip* decides, not the microsecond of arrival. The tick length is an operating parameter of the Block Engine and is tuned to bundle volume.
...
Losing bundles are not discarded immediately: they re-enter the next tick and keep competing for several seconds before expiring, so contending bundles serialize across ticks instead of being lost.
